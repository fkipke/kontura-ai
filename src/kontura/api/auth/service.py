...
- Service wirft DOMAIN-Exceptions (kein HTTP-Wissen).
"""
...
class AuthService:
    ...
    async def register(self, payload: RegisterRequest) -> User:
        ...
        Raises:
            ConflictError: wenn der Tenant-Slug schon existiert.
        """
        existing_tenant = await self._tenants.get_by_slug(payload.tenant_slug)
        if existing_tenant is not None:
            raise ConflictError(f"Tenant-Slug '{payload.tenant_slug}' existiert bereits.")
    ...
    async def login(self, payload: LoginRequest) -> User:
        ...
        Raises:
            UnauthorizedError: bei jedem Login-Fehler.
        """
